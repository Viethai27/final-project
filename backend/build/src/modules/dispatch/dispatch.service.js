...
exports.createDispatchDecision = exports.getDispatchDecisionById = exports.getDispatchDecisions = exports.getDispatchSuggestionByVisitId = exports.getDispatchSuggestions = exports.getDispatchStage = exports.suggestionFromVisit = void 0;
...
const mapDecisionRecommendation = (recommendation) => ({
...
const mapDecisionOutcome = (outcome) => outcome
    ? {
...
const suggestionFromVisit = async (visit) => {
    const stage = (0, exports.getDispatchStage)(visit);
    const roomTypes = stageRoomTypes[stage] ?? ['EXAM'];
    const preferredService = getPreferredService(visit, stage);
    const candidateRooms = await prisma_1.prisma.room.findMany({
        where: {
            isActive: true,
            roomType: {
                in: roomTypes,
    ...
    const [loads, queueItems, doctors, serviceRooms] = await Promise.all([
        prisma_1.prisma.resourceLoad.findMany({
            where: {
                roomId: { in: roomIds },
                isActive: true,
            },
        ...
        prisma_1.prisma.queueItem.findMany({
            where: {
        ...
        prisma_1.prisma.serviceRoom.findMany({
            where: {
                roomId: { in: roomIds },
                isActive: true,
            },
    ...
    const queueLengthByRoom = queueItems.reduce((acc, item) => {
    ...
    const latestLoadByRoom = loads.reduce((acc, load) => {
        if (!acc[load.roomId]) {
            acc[load.roomId] = load;
        }
        return acc;
    }, {});
    const serviceByRoom = serviceRooms.reduce((acc, item) => {
        if (!acc[item.roomId]) {
    ...
    return candidateRooms
        ...
        const doctor = room.roomType === 'EXAM'
...
exports.suggestionFromVisit = suggestionFromVisit;
const getDispatchStage = (visit) => {
...
exports.getDispatchStage = getDispatchStage;
const getPreferredService = (visit, stage) => {
    if (stage === 'CLS') {
        const pendingCls = visit.clsOrders.find((order) => ['PENDING', 'ASSIGNED', 'IN_PROGRESS'].includes(order.status));
...
const mapSuggestion = async (visit) => {
    const candidates = await (0, exports.suggestionFromVisit)(visit);
    return {
        visit: mapVisitSummary(visit),
        stage: (0, exports.getDispatchStage)(visit),
        queueItem: visit.queueItems?.[0] ? mapQueueItem(visit.queueItems[0]) : null,
        candidates,
    };
};
const getDispatchSuggestions = async (query) => {
    const where = {
        progress: {
            is: {
                currentState: {
                    in: [...activeVisitStates],
                },
            },
        },
    };
...
exports.getDispatchSuggestions = getDispatchSuggestions;
const getDispatchSuggestionByVisitId = async (visitId) => {
...
exports.getDispatchSuggestionByVisitId = getDispatchSuggestionByVisitId;
const getDispatchDecisions = async (query) => {
    const where = {};
    if (query.search) {
        where.OR = [
    ...
    const [total, decisions] = await prisma_1.prisma.$transaction([
        prisma_1.prisma.dispatchDecision.count({ where }),
        prisma_1.prisma.dispatchDecision.findMany({
            where,
...
exports.getDispatchDecisions = getDispatchDecisions;
const getDispatchDecisionById = async (id) => {
    const decision = await prisma_1.prisma.dispatchDecision.findUnique({
        where: { id },
        select: dispatchDecisionSelect,
    });
    if (!decision) {
        throw new http_error_1.AppError('Dispatch decision not found.', 404);
    }
    return {
        dispatchDecisionId: decision.id,
        visitId: decision.visitId,
...
exports.getDispatchDecisionById = getDispatchDecisionById;
const resolveProvidedRoom = async (roomId) => {
    if (!roomId) {
        return null;
    }
    ...
    if (!room) {
        throw new http_error_1.AppError('Room not found.', 404);
    }
    return room.id;
};
const resolveProvidedDoctor = async (doctorId) => {
    if (!doctorId) {
        return null;
    }
    ...
    if (!doctor) {
        throw new http_error_1.AppError('Doctor not found.', 404);
    }
    return doctor.id;
};
const resolveProvidedUser = async (userId) => {
    if (!userId) {
        return null;
    }
    ...
    if (!user) {
        throw new http_error_1.AppError('User not found.', 404);
    }
    return user.id;
};
const resolveProvidedService = async (serviceId) => {
    if (!serviceId) {
        return null;
    }
    const service = await prisma_1.prisma.serviceCatalog.findUnique({
        where: { id: serviceId },
        select: {
            id: true,
        },
    ...
    if (!service) {
        throw new http_error_1.AppError('Service not found.', 404);
    }
    return service.id;
};
const resolveVisitForDecision = async (visitId) => {
    const visit = await prisma_1.prisma.visit.findUnique({
        where: { id: visitId },
        select: listVisitSuggestionsSelect,
    });
...
const createDispatchDecision = async (input) => {
    ...
    if (queueItemId) {
        ...
        if (queueItem.visitId !== visit.id) {
            throw new http_error_1.AppError('Queue item does not belong to the visit.', 400);
        }
    }
    ...
    const suggestions = await (0, exports.suggestionFromVisit)(visit);
    const recommendationInputs = input.recommendations && input.recommendations.length > 0
        ...
        })).filter((recommendation) => Boolean(recommendation.roomId));
    ...
    const inferredOutcomeServiceId = await resolveProvidedService(input.outcome?.serviceId ?? input.serviceId ?? visit.appointment?.service?.id ?? null);
    ...
    const waitDifference = input.outcome?.waitDifference ??
    ...
    const created = (await prisma_1.prisma.$transaction(async (tx) => {
        ...
        if (existingDecision) {
            throw new http_error_1.AppError('Dispatch decision already exists for this visit.', 409);
        }
        return tx.dispatchDecision.create({
            data: {
                ...
                queueItemId,
                decisionById,
                decisionType: (input.decisionType ?? 'SYSTEM_SUGGESTED'),
                outcomeRoomId,
                outcomeDoctorId,
                ...
                outcome: {
                    create: {
                        serviceId: (input.outcome?.serviceId ?? inferredOutcomeServiceId ?? null),
...
exports.createDispatchDecision = createDispatchDecision;
