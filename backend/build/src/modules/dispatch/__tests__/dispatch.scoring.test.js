...
const createDoctor = (id, defaultRoomId = 'room-current') => ({
    id,
    name: `Bac si ${id}`,
...
(0, vitest_1.describe)('TC-D1..TC-D7: dispatch suggestion scoring', () => {
    (0, vitest_1.it)('TC-D1: phong co queue ngan hon duoc xep cao hon', async () => {
        // Nghiep vu: phong co hang cho ngan hon phai co diem cao hon.
        prisma_1.prisma.visit.findUnique.mockResolvedValueOnce(createVisit());
        prisma_1.prisma.room.findMany.mockResolvedValueOnce([
            createRoom('room-a'),
            createRoom('room-b'),
        ]);
        prisma_1.prisma.resourceLoad.findMany.mockResolvedValueOnce([
            {
                roomId: 'room-a',
                currentLoad: 1,
                queueLength: 1,
        ...
        prisma_1.prisma.doctorProfile.findMany.mockResolvedValueOnce([]);
        prisma_1.prisma.serviceRoom.findMany.mockResolvedValueOnce([
            { roomId: 'room-a', service: createService('svc-a') },
            { roomId: 'room-b', service: createService('svc-b') },
        ]);
        const result = await (0, dispatch_service_1.getDispatchSuggestionByVisitId)('visit-1');
        (0, vitest_1.expect)(result.candidates[0].room?.id).toBe('room-a');
        (0, vitest_1.expect)(result.candidates[0].resourceScore).toBeGreaterThan(result.candidates[1].resourceScore);
    });
    (0, vitest_1.it)('TC-D2: penalty overload lam giam diem manh hon normal', async () => {
        ...
        (0, vitest_1.expect)(normal.resourceScore).toBeGreaterThan(overload.resourceScore);
        (0, vitest_1.expect)(Math.round((normal.resourceScore - overload.resourceScore) * 100)).toBeGreaterThanOrEqual(18);
    });
    (0, vitest_1.it)('TC-D3: WARNING nhe hon OVERLOAD', async () => {
        ...
        (0, vitest_1.expect)(normal.resourceScore).toBeGreaterThan(warning.resourceScore);
        (0, vitest_1.expect)(warning.resourceScore).toBeGreaterThan(overload.resourceScore);
    });
    (0, vitest_1.it)('TC-D4: phong hien tai nhan bonus quay lai dung phong', async () => {
    ...
    (0, vitest_1.it)('TC-D5: estimated wait tinh dung theo cong thuc', async () => {
        // Nghiep vu: thoi gian cho uoc tinh phai dung cong thuc cua service.
        prisma_1.prisma.visit.findUnique.mockResolvedValueOnce(createVisit());
        prisma_1.prisma.room.findMany.mockResolvedValueOnce([createRoom('room-a', 'EXAM', 15)]);
        prisma_1.prisma.resourceLoad.findMany.mockResolvedValueOnce([
            {
                roomId: 'room-a',
                currentLoad: 4,
        ...
        const result = await (0, dispatch_service_1.getDispatchSuggestionByVisitId)('visit-1');
        (0, vitest_1.expect)(result.candidates[0].estimatedWaitMinutes).toBe(38);
    });
    (0, vitest_1.it)('TC-D6: score khong bao gio am', async () => {
        // Nghiep vu: diem dieu phoi khong duoc am.
        prisma_1.prisma.visit.findUnique.mockResolvedValueOnce(createVisit());
        prisma_1.prisma.room.findMany.mockResolvedValueOnce([createRoom('room-a')]);
        prisma_1.prisma.resourceLoad.findMany.mockResolvedValueOnce([
            {
                roomId: 'room-a',
                currentLoad: 20,
        ...
        const result = await (0, dispatch_service_1.getDispatchSuggestionByVisitId)('visit-1');
        (0, vitest_1.expect)(result.candidates[0].resourceScore).toBeGreaterThanOrEqual(0);
    });
    (0, vitest_1.it)('TC-D7: khong co candidate room thi tra ve mang rong', async () => {
