...
(0, vitest_1.describe)('TC-Q1..TC-Q7: queue sorting and filtering', () => {
    (0, vitest_1.it)('TC-Q1: WAITING luon nam truoc DONE', async () => {
        ...
        (0, vitest_1.expect)(result.items[0].queueItemId).toBe('waiting-item');
        (0, vitest_1.expect)(result.items[1].queueItemId).toBe('done-item');
    });
    (0, vitest_1.it)('TC-Q2: trong cung WAITING, PRIORITY nam truoc NORMAL', async () => {
        ...
        const result = await (0, queue_service_1.getQueueItems)({ page: 1, limit: 10, sort: 'desc', status: 'ALL' });
        (0, vitest_1.expect)(result.items[0].queueItemId).toBe('priority-item');
        (0, vitest_1.expect)(result.items[1].queueItemId).toBe('normal-item');
    });
    (0, vitest_1.it)('TC-Q3: thu tu lane PRIORITY > APPOINTMENT > AFTER_CLS > NORMAL', async () => {
        ...
        const result = await (0, queue_service_1.getQueueItems)({ page: 1, limit: 10, sort: 'desc', status: 'ALL' });
        (0, vitest_1.expect)(result.items.map(item => item.queueItemId)).toEqual([
            'priority-item',
            'appointment-item',
            'after-cls-item',
            'normal-item',
    ...
    (0, vitest_1.it)('TC-Q4: trong cung lane, diem cao hon len truoc', async () => {
        ...
        const result = await (0, queue_service_1.getQueueItems)({ page: 1, limit: 10, sort: 'desc', status: 'ALL' });
        (0, vitest_1.expect)(result.items[0].queueItemId).toBe('score-95');
        (0, vitest_1.expect)(result.items[1].queueItemId).toBe('score-80');
    });
    (0, vitest_1.it)('TC-Q5: pagination dung voi page=2 limit=2', async () => {
        ...
        const result = await (0, queue_service_1.getQueueItems)({ page: 2, limit: 2, sort: 'desc', status: 'ALL' });
        (0, vitest_1.expect)(result.items).toHaveLength(2);
        (0, vitest_1.expect)(result.total).toBe(5);
    });
    (0, vitest_1.it)("TC-Q6: filter status='WAITING' chi lay WAITING items", async () => {
        ...
        await (0, queue_service_1.getQueueItems)({ page: 1, limit: 10, sort: 'desc', status: 'WAITING' });
        const call = prisma_1.prisma.queueItem.findMany.mock.calls[0][0];
        (0, vitest_1.expect)(call.where.status.is.status).toBe('WAITING');
    });
    (0, vitest_1.it)('TC-Q7: getQueueItemById voi id khong ton tai phai throw 404', async () => {
        // Nghiep vu: tim queue item khong co phai tra loi 404 ro rang.
        prisma_1.prisma.queueItem.findUnique.mockResolvedValueOnce(null);
        await (0, vitest_1.expect)((0, queue_service_1.getQueueItemById)('missing-id')).rejects.toMatchObject({
            statusCode: 404,
        });
    });
});
