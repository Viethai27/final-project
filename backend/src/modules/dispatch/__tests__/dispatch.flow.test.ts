...
import { getQueueItems } from '../../queue/queue.service';
import { getDispatchStage, suggestionFromVisit } from '../dispatch.service';
import {
  rankRoomSuggestions,
  runSimulation,
  type SimulatedRoom,
  type SimulationConfig,
} from '../../../simulation/dispatch-simulation';
...
const createVisit = (currentState: string, extra: Partial<any> = {}) => ({
  id: 'visit-1',
  queueNumber: 'V-001',
...
const createQueueItem = (
  id: string,
  laneType: 'NORMAL' | 'PRIORITY' | 'APPOINTMENT' | 'AFTER_CLS',
...
const makeSimRooms = (overrides: Partial<SimulatedRoom> = {}): SimulatedRoom[] => [
  {
    id: 'room-a',
    name: 'Phong A',
    roomType: 'EXAM',
...
const installDispatchMocks = (visit: ReturnType<typeof createVisit>, roomPool: typeof roomMap | Record<string, any>) => {
  prisma.visit.findUnique.mockResolvedValue(visit);
  prisma.room.findMany.mockImplementation(async ({ where }: any) => {
    const allowedTypes: string[] = where?.roomType?.in ?? ['EXAM'];
    return Object.values(roomPool)
      .filter((room: any) => allowedTypes.includes(room.roomType))
      .map((room: any) => ({ ...room }));
  });
  prisma.resourceLoad.findMany.mockImplementation(async ({ where }: any) => {
    const roomIds = where?.roomId?.in ?? [];
    return Object.values(roomPool)
      .filter((room: any) => roomIds.includes(room.id))
      .map((room: any) => ({
        roomId: room.id,
  ...
  prisma.queueItem.findMany.mockImplementation(async ({ where }: any) => {
    const roomIds = where?.targetRoomId?.in ?? [];
    return Object.values(roomPool)
      .filter((room: any) => roomIds.includes(room.id))
      .flatMap((room: any) => Array.from({ length: room.queueLength ?? 0 }, (_, index) => ({
        targetRoomId: room.id,
        order: index,
      })));
  });
  prisma.doctorProfile.findMany.mockResolvedValue([]);
  prisma.serviceRoom.findMany.mockResolvedValue([]);
};

describe('Luong nghiep vu & hieu qua gia lap', () => {
  it('KC-1: luong co ban EXAM -> CLS -> CONCLUSION', async () => {
    ...
    const step1 = await suggestionFromVisit(visitExam);
    expect(getDispatchStage(visitExam)).toBe('EXAM');
    expect(step1[0].room?.id).toBe('room-a');

    const visitCls = createVisit('WAITING_CLS');
    installDispatchMocks(visitCls, roomMap);
    const step2 = await suggestionFromVisit(visitCls);
    expect(getDispatchStage(visitCls)).toBe('CLS');
    expect(step2.every(candidate => ['LAB', 'IMAGING'].includes(candidate.room?.roomType ?? ''))).toBe(true);

    const visitConclusion = createVisit('WAITING_CONCLUSION');
    installDispatchMocks(visitConclusion, roomMap);
    const step3 = await suggestionFromVisit(visitConclusion);
    expect(getDispatchStage(visitConclusion)).toBe('CONCLUSION');
    expect(step3.every(candidate => candidate.room?.roomType === 'EXAM')).toBe(true);
  });

  it('KC-2: benh nhan uu tien chen ngang hang doi', async () => {
    ...
    expect(result.items.map(item => item.queueItemId)).toEqual(['c', 'd', 'a', 'b']);
  });

  it('KC-3: benh nhan quay lai sau CLS o lan AFTER_CLS', async () => {
    ...
    const rooms: Record<string, any> = {
      ...roomMap,
      examA: { ...roomMap.examA, queueLength: 1, utilizationRate: 0.35, alertLevel: 'NORMAL' as const },
      examB: { ...roomMap.examB, queueLength: 4, utilizationRate: 0.45, alertLevel: 'NORMAL' as const },
    };
    installDispatchMocks(visit, rooms);

    const result = await suggestionFromVisit(visit);
    expect(getDispatchStage(visit)).toBe('CONCLUSION');
    expect(visit.progress.sameDoctorRequired).toBe(true);
    expect(result[0].room?.id).toBe('room-a');
    expect(result.every(candidate => candidate.room?.roomType === 'EXAM')).toBe(true);
  });

  it('KC-4: tat ca phong overload van con fallback cap 1', async () => {
    ...
    prisma.visit.findUnique.mockResolvedValue(visit);
    prisma.room.findMany.mockResolvedValue(roomsForFallbackScenario.filter(room => room.utilizationRate < 1.2));
    prisma.resourceLoad.findMany.mockResolvedValue([
    ...
    const result = await suggestionFromVisit(visit);
    expect(result.map(candidate => candidate.room?.id)).toEqual(['room-a', 'room-b']);
    expect(result[0].alertLevel).toBe('OVERLOAD');
  });

  it('KC-5: fallback khong con phong thi tra ve mang rong', async () => {
  ...
  it('KC-6: cao diem dua tren peak/off-peak va so sanh co he thong voi random', async () => {
    // Nghiep vu: khi cao diem, phuong an co he thong phai tot hon random va khong lam tang overload.
    const peakRooms: SimulatedRoom[] = [
    ...
    const offPeakRooms: SimulatedRoom[] = [
    ...
    const peakRank = rankRoomSuggestions(peakRooms, {});
    const offPeakRank = rankRoomSuggestions(offPeakRooms, {});
    expect(peakRank[0].room.id).toBe('room-c');
    expect(offPeakRank[0].room.id).toBe('room-c');
    expect(peakRank[0].estimatedWaitMinutes).toBeGreaterThan(offPeakRank[0].estimatedWaitMinutes);
