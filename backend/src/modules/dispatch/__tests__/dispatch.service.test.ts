...
const createDoctor = (id: string, defaultRoomId = 'room-a') => ({
  id,
  name: `Bac si ${id}`,
...
beforeEach(() => {
  ...
  prisma.visit.findUnique.mockResolvedValue(createVisit());
  prisma.room.findMany.mockResolvedValue([createRoom('room-a')]);
  ...
  prisma.user.findUnique.mockImplementation(async ({ where }: any) => ({ id: where.id }));
  prisma.room.findUnique.mockImplementation(async ({ where }: any) => ({ id: where.id }));
  prisma.doctorProfile.findUnique.mockImplementation(async ({ where }: any) => ({ id: where.id }));
  prisma.serviceCatalog.findUnique.mockImplementation(async ({ where }: any) => ({ id: where.id }));
  prisma.queueItem.findUnique.mockImplementation(async ({ where }: any) => ({ id: where.id, visitId: 'visit-1' }));
...
describe('Dispatch service coverage helpers', () => {
  it('getDispatchDecisions maps list result', async () => {
    // Nghiep vu: danh sach quyet dinh dieu phoi phai map dung ve DTO.
    prisma.dispatchDecision.count.mockResolvedValueOnce(1);
    prisma.dispatchDecision.findMany.mockResolvedValueOnce([createDispatchDecisionRecord()]);

    const result = await getDispatchDecisions({
      page: 1,
      limit: 10,
      sort: 'desc',
      search: undefined,
    ...
    expect(result.items[0].dispatchDecisionId).toBe('dec-1');
    expect(result.items[0].recommendations).toHaveLength(1);
  });

  it('getDispatchDecisionById maps detail result', async () => {
    // Nghiep vu: lay chi tiet quyet dinh phai tra ve du thong tin can demo.
    prisma.dispatchDecision.findUnique.mockResolvedValueOnce(createDispatchDecisionRecord());

    const result = await getDispatchDecisionById('dec-1');

    expect(result.dispatchDecisionId).toBe('dec-1');
    expect(result.outcome?.dispatchOutcomeId).toBe('out-1');
    expect(result.visit.visitId).toBe('visit-1');
  });

  it('createDispatchDecision creates recommendation and outcome', async () => {
    // Nghiep vu: tao quyet dinh hop le phai tao du recommendation va outcome.
    const result = await createDispatchDecision({
      visitId: 'visit-1',
      queueItemId: 'qi-1',
      decisionById: 'user-1',
      decisionType: 'MANUAL',
    ...
    expect(result.outcome?.dispatchOutcomeId).toBe('out-1');
    expect(txMock.dispatchDecision.create).toHaveBeenCalled();
  });

  it('createDispatchDecision throws 409 when decision already exists', async () => {
