...
const addMinutes = (date: Date | null | undefined, minutes: number | null | undefined) => {
...
const pickVisitDoctor = (visit: any) => {
  const currentAssignment = visit.assignments?.find((assignment: any) => assignment.isCurrent);
  const latestTurn = visit.turns?.[0];

  return visit.appointment?.doctor ?? currentAssignment?.doctor ?? latestTurn?.doctor ?? null;
};

const pickVisitRoom = (visit: any) => {
  const currentAssignment = visit.assignments?.find((assignment: any) => assignment.isCurrent);
  const latestTurn = visit.turns?.[0];
...
const pickVisitDepartment = (visit: any) => {
...
const getVisitExpectedFinishTime = (visit: any) => {
  const serviceDuration =
    visit.appointment?.service?.avgDuration ??
...
const mapVisitListItem = (visit: any) => {
...
const mapVisitStateHistory = (history: any) => ({
  id: history.id,
...
const mapVisitQueueItem = (queueItem: any) => ({
  ...
  status: queueItem.status
    ...
    : null,
  histories: (queueItem.histories ?? []).map((history: any) => ({
    id: history.id,
...
const mapVisitClsOrder = (order: any) => ({
  clsOrderId: order.id,
...
const mapVisitDispatchDecision = (decision: any) => ({
  ...
  recommendations: (decision.recommendations ?? []).map((recommendation: any) => ({
...
const mapVisitInvoice = (invoice: any) => ({
...
const mapVisitDetail = (visit: any) => ({
  visitId: visit.id,
  queueNumber: visit.queueNumber,
  patient: mapPatient(visit.patient),
  ...
  clinical: visit.clinical
    ...
    : null,
  assignments: (visit.assignments ?? []).map((assignment: any) => ({
  ...
  escalationLogs: (visit.escalationLogs ?? []).map((log: any) => ({
...
const resolveWalkInDoctor = async (tx: Prisma.TransactionClient, doctorId: string | null) => {
  if (!doctorId) {
    return null;
  }
...
const resolveWalkInService = async (tx: Prisma.TransactionClient, serviceId: string | null) => {
  if (!serviceId) {
    return null;
  }
...
const resolveWalkInRoom = async (
  ...
  if (input.doctorDefaultRoomId) {
    ...
    if (!room || !room.isActive) {
      throw new AppError('Phòng khám của bác sĩ không còn khả dụng.', 404);
    }
    ...
    if (input.roomTypeRequired && room.roomType !== input.roomTypeRequired) {
      throw new AppError('Phòng khám không phù hợp với dịch vụ đã chọn.', 409);
    }
  ...
  const room = await tx.room.findFirst({
    where: {
      ...
      roomType: (input.roomTypeRequired ?? 'EXAM') as any,
      isActive: true,
    },
  ...
  if (!room) {
    throw new AppError('Không tìm thấy phòng khám phù hợp cho lượt đăng ký này.', 400);
  }

  return room.id;
};

const ensureInvoiceForVisit = async (tx: Prisma.TransactionClient, visitId: string) => {
  ...
  ].filter((service): service is { id: string; name: string; price: Prisma.Decimal | null } => Boolean(service));
...
export const createWalkInVisit = async (input: {
  ...
  if (!input.visit.departmentId) {
    throw new AppError('Khoa khám là bắt buộc.', 400);
  }
  ...
  const createdVisitId = await prisma.$transaction(async tx => {
    ...
    const laneType = deriveLaneType(priorityReason, false);
  ...
  return getVisitById(createdVisitId);
};

export const concludeVisit = async (
  id: string,
  payload: {
  ...
  if (!payload.finalDiagnosis.trim()) {
    throw new AppError('Final diagnosis is required.', 400);
  }

  if (!payload.conclusion.trim()) {
    throw new AppError('Conclusion is required.', 400);
  }
...
export const getVisits = async (query: ListQueryParams) => {
